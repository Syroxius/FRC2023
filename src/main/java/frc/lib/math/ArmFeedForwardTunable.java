 ...
 */
public class ArmFeedForwardTunable {
    public double ks;
    public double kg;
    public double kv;
    public double ka;
     ...
     */
    public ArmFeedForwardTunable(double ks, double kg, double kv, double ka) {
        this.ks = ks;
        this.kg = kg;
        this.kv = kv;
        this.ka = ka;
    ...
    /**
     * Creates a new ArmFeedforward with the specified gains. Acceleration gain is defaulted to
     ...
     * @param kv The velocity gain.
     */
    public ArmFeedForwardTunable(double ks, double kg, double kv) {
        this(ks, kg, kv, 0);
    }
     ...
     *        horizontal (i.e. if the provided angle is 0, the arm should be parallel with the
    ...
    public double calculate(double positionRadians, double velocityRadPerSec,
        ...
        return ks * Math.signum(velocityRadPerSec) + kg * Math.cos(positionRadians)
            + kv * velocityRadPerSec + ka * accelRadPerSecSquared;
    }
     ...
     *        the provided angle is 0, the arm should be parallel with the floor). If your encoder
     *        does not follow this convention, an offset should be added.
     ...
     * and a velocity. Useful for ensuring that velocity and acceleration constraints for a
    ...
    public double maxAchievableAcceleration(double maxVoltage, double angle, double velocity) {
        return (maxVoltage - ks * Math.signum(velocity) - Math.cos(angle) * kg - velocity * kv)
            / ka;
    }
